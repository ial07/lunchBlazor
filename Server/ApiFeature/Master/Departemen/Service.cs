...
namespace RepositoryPattern.Services.DepartemenService
{
    public class DepartemenService : IDepartemenService
    {
        ...
        public async Task<PageList<Departemen>> Get(SieveModel model)
        {
            try
            {
                var departemen = _AppDbContext.Departemen.Where(d => d.IsActive == true).AsQueryable();
        ...
        public async Task<Object> Put(Guid id, UpdateDto items)
        {
            try
            {
                ...
                var checkData = await _AppDbContext.Departemen.FirstOrDefaultAsync(x => x.Name == items.Name & roleData.Name != items.Name);
                if (checkData != null)
                {
