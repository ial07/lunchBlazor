...
namespace test_blazor.Server.Controllers
{
    ...
    public class JenisMppController : ControllerBase
    {
        ...
        public JenisMppController(SieveProcessor sieveProcessor, AppDbContext AppDbContext)
        {
        ...
        [HttpGet]
        public async Task<ActionResult<List<JenisMpp>>> GetDevisis([FromQuery] SieveModel model)
        {

            var Devisi = _AppDbContext.JenisMpp.Where(d => (bool)d.IsActive).AsQueryable();
            var result = _SieveProcessor.Apply(model, Devisi);
            var devisi = await PageList<JenisMpp>.ShowDataAsync(
                Devisi,
                result,
                model.Page,
                model.PageSize
        ...
        public async Task<IActionResult> PostDevisi([FromBody] CreateDevisiInput items)
        {
            try
            {
                ...
                await _AppDbContext.JenisMpp.AddAsync(roleData);
