...
namespace blazor.Server.Controllers
{
    ...
    public class GolonganController : ControllerBase
    {
        ...
        public GolonganController(IGolonganService GolonganService, ValidationDto MasterValidationService)
        {
            _IGolonganService = GolonganService;
        ...
        public async Task<IActionResult> GetId([FromRoute] Guid id)
        {
            try
            {
                var dataList = await _IGolonganService.GetId(id);
                return Ok(dataList);
            }
            catch (CustomException ex)
            {
