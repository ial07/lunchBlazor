...
namespace RepositoryPattern.Services.PosisiService
{
    public class PosisiService : IPosisiService
    {
        ...
        public async Task<PageList<Posisi>> Get(SieveModel model)
        {
            try
            {
                var Posisi = _AppDbContext.Posisi.Where(d => d.IsActive == true).AsQueryable();
                var result = _SieveProcessor.Apply(model, Posisi);
                var PosisiList = await PageList<Posisi>.ShowDataAsync(
                    Posisi,
                    result,
                    model.Page,
                    model.PageSize
        ...
        public async Task<Object> GetId(Guid Id)
        {
            try
            {
                var checkData = await _AppDbContext.Posisi.FirstOrDefaultAsync(x => x.Id == Id);
                if (checkData == null)
                {
        ...
        public async Task<Object> Put(Guid id, UpdateDto items)
        {
            try
            {
                var roleData = await _AppDbContext.Posisi.FindAsync(id) ?? throw new CustomException(400, "Data tidak ditemukan"); ;
                var checkData = await _AppDbContext.Posisi.FirstOrDefaultAsync(x => x.Name == items.Name & roleData.Name != items.Name);
                if (checkData != null)
                {
