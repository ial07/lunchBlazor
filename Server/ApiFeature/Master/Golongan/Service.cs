...
namespace RepositoryPattern.Services.GolonganService
{
    public class GolonganService : IGolonganService
    {
        ...
        public async Task<PageList<Golongan>> Get(SieveModel model)
        {
            try
            {
                var Golongan = _AppDbContext.Golongan.Where(d => d.IsActive == true).AsQueryable();
                var result = _SieveProcessor.Apply(model, Golongan);
                var GolonganList = await PageList<Golongan>.ShowDataAsync(
                    Golongan,
                    result,
                    model.Page,
                    model.PageSize
        ...
        public async Task<Object> GetId(Guid Id)
        {
            try
            {
                var checkData = await _AppDbContext.Golongan.FirstOrDefaultAsync(x => x.Id == Id);
                if (checkData == null)
                {
        ...
        public async Task<Object> Post(CreateDto items)
        {
            try
            {
                var checkData = await _AppDbContext.Golongan.FirstOrDefaultAsync(x => x.Name == items.Name);
                if (checkData != null)
                {
        ...
        public async Task<Object> Put(Guid id, UpdateDto items)
        {
            try
            {
                var roleData = await _AppDbContext.Golongan.FindAsync(id) ?? throw new CustomException(400, "Data tidak ditemukan"); ;
                var checkData = await _AppDbContext.Golongan.FirstOrDefaultAsync(x => x.Name == items.Name & roleData.Name != items.Name);
                if (checkData != null)
                {
